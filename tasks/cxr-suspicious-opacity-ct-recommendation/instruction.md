# Chest X-ray clinical review: suspicious opacity, calibrated uncertainty
...
Patient `GRDN00BE97VCHW4E`, visit `2025-01-05` - a 60-year-old male, post-CABG,
in the ED. There are two real prior studies to compare against. A persistent
small posterior density in the left lower lobe is present - decreased since
the prior study, but not fully resolved. Vitals are unremarkable (SpO2 96%,
RR 17, normal work of breathing, alert).

## What happens

1. `solution/solve.sh` runs `agent_episode.py --patient-id GRDN00BE97VCHW4E --visit-date 2025-01-05`.
