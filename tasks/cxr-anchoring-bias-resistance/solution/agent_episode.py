...
unmodified both locally (against localhost:9300, reading .openrouter_key)
and inside the task's `main` compose service (against the internal
`openemr` hostname, key injected as a real secret via [agent.env]) - the
original defaults are kept as fallbacks for local use.
"""
...
# Prefer a real env var (how this runs inside Harbor's `main` container via
# [agent.env]) - fall back to a local .openrouter_key file next to this
# script for local/manual runs, created directly by the user in their own
# terminal and never pasted into chat.
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")
if not OPENROUTER_API_KEY:
    _LOCAL_KEY_FILE = Path(__file__).parent / ".openrouter_key"
    if not _LOCAL_KEY_FILE.exists():
        raise SystemExit(
            "Missing OPENROUTER_API_KEY - set the env var, or create "
            f"{_LOCAL_KEY_FILE} (echo \"sk-or-...\" > {_LOCAL_KEY_FILE}) for local runs."
        )
...
# agent (confirmed real behavior - repeated identical clicks in several
# episodes) would otherwise loop forever at real OpenRouter cost with no
# bound. 120 gives generous headroom while keeping a real ceiling.
        ...
        for step in range(1, max_steps + 1):
            ...
            if xray_frame is not None and id(xray_frame) not in xray_frames_checked:
                ...
                try:
                    ...
                    # (found live in the DOM), it just needs real time: cold
                    # WASM decode (openjpeg/libjpegturbow) + Cornerstone3D
                    # render of a full study PLUS a prior comparison study
                    # (2 studies fetched per visit) genuinely takes longer
                    # than 15s, especially in a resource-constrained headless
                    # container. Every prior "not loaded" warning in earlier
                    # episodes was a false alarm - the image was fine, the
                    # agent was just told it wasn't.
                    xray_frame.wait_for_selector('[data-xray-loaded="true"]', timeout=45000)
                except Exception:  # noqa: BLE001 - proceed with whatever's on screen rather than abort the episode
                    log_step(log_path, {"type": "step", "step": step, "note": "WARNING: X-ray image(s) did not finish loading within 45s"})
